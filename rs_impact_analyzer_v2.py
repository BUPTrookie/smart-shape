...
⚠️ 重要结论（代码审查 #1 修正后）：
    早期版本曾声称「距离核优化使 RMSE 下降」，那是 scaler 在全量数据上 fit +
    无 hold-out 造成的数据泄露假象。修正泄露（Pipeline 每折独立 fit + 留出测试集）
    后，可信 hold-out 指标显示：**距离核优化并无改进**——基线(位置桶特征)
    RMSE≈0.0117，距离核优化 RMSE≈0.0129，反而略升。即基线 26 特征已足够，
    距离核属探索性尝试，结论为「不采纳」。结题报告以 ``rs_impact_analyzer.py``(v1)
    的可信 hold-out 指标(R²≈0.88)为准，不引用本文件的优化结论。
