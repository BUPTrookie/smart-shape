...
"""
Excel所有工作表最终处理工具
处理Excel文件中的4个指定工作表，将状态为Pre的数据进行字段转移
"""
...
def process_excel_sheets(input_file: str, output_file: str = None) -> None:
    ...
    # 根据实际工作表名称进行精确匹配
    target_keywords = [
        "含重复",      # 匹配 "Reshaping (含重复)"
        "去重复",      # 匹配 "Reshaping (去重复)"
        "去重复去跳点", # 匹配 "Reshaping (去重复去跳点)"
        "NG"          # 匹配 "Reshaping (NG)"
    ]
    ...
    for sheet_name in available_sheets:
        # 跳过"Reshaping"（无括号版本）和"透明图"
        if sheet_name == "Reshaping" or "透明图" in sheet_name:
            continue
