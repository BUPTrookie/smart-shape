...
def generate_detailed_comparison_lines():
    ...
    # 读取处理结果
    comparison_file = "Output/reshaping_pre_comparison.csv"
    df = pd.read_csv(comparison_file)
    print(f"读取对比数据: {len(df)} 行")

    # 创建输出目录
    output_dir = "charts/detailed_comparison_lines"
...
def create_shape_detailed_lines_chart(df, shape, output_dir):
    ...
    if len(gen_data) > 0:
        ...
        ax2.set_title(f'Shape {shape} - 生成结果 ({len(gen_data)} 条曲线)', fontsize=14, fontweight='bold')
    ...
    ax3.set_ylabel('FAI156值')
    ax3.legend()
    ax3.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(f"{output_dir}/Shape_{shape}_detailed_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_summary_comparison_chart(df, all_shapes, output_dir):
    ...
    # 创建一个大的汇总图
    fig, axes = plt.subplots(len(all_shapes)//4 + 1, 4, figsize=(20, 4 * (len(all_shapes)//4 + 1)))
    if len(all_shapes) <= 4:
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/all_shapes_summary.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def generate_statistics_report(df, output_dir):
    ...
    print(f"统计报告已保存到: {output_dir}/detailed_comparison_report.md")
...
if __name__ == "__main__":
    ...
    # 生成统计报告
    df = pd.read_csv("Output/reshaping_pre_comparison.csv")
    output_dir = "charts/detailed_comparison_lines"
