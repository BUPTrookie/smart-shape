...
def split_1209_by_shape():
    """根据Shape类型分组,输出不同的CSV文件"""

    try:
        # 读取数据
        logger.info("正在读取 test_1209_DZ_result.csv ...")
        df = pd.read_csv("test_1209_DZ_result.csv")
        logger.info(f"成功读取 {len(df)} 条记录")

        # 获取所有Shape类型
        shape_types = df['Shape'].unique()
        logger.info(f"发现 {len(shape_types)} 种不同的Shape类型")

        # 创建输出目录
        output_dir = "Output/dz_group_by_shape_1209"
        ...
        for shape_type in sorted(shape_types):
            # 筛选该类型的数据
            group_df = df[df['Shape'] == shape_type]
        ...
        for stats in group_stats_sorted:
            print(f"  {stats['shape_type']}: {stats['count']} 条 ({stats['percentage']:.2f}%)")
