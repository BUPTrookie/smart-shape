...
class V4ShapeVisualizer:
    ...
    def create_shape_visualization(self, df, shape_type):
        """为特定shape_v4类型创建可视化"""
        # 筛选该shape的数据
        shape_data = df[df['shape_v4'] == shape_type].copy()
        ...
        # 设置标题和标签
        ax.set_title(f'V4 Shape: {shape_type} ({len(shape_data)} 条数据)',
        ...
        # 在右上角添加shape信息
        ax.text(0.95, 0.95, f'V4 Shape: {shape_type}',
        ...
        # 保存图像
        filename = f'v4_shape_{shape_type}.png'
    ...
    def create_summary_visualization(self, df):
        """创建shape分布汇总图"""
        shape_counts = df['shape_v4'].value_counts()
        ...
        ax1.set_title('V4 Shape分布 (所有类型)', fontsize=14, fontweight='bold')
        ...
        ax2.set_title('V4 Shape分布占比 (前8个)', fontsize=14, fontweight='bold')
        ...
        # 保存汇总图
        summary_filepath = os.path.join(self.output_dir, 'v4_shape_summary.png')
    ...
    def create_physical_category_analysis(self, df):
        ...
        fig.suptitle('V4版本第三段物理分类分析', fontsize=16, fontweight='bold')
        ...
        for i, (char, physical_name) in enumerate(physical_map.items()):
            ...
            # 筛选包含该物理分类的shape数据
            physical_shapes = [shape for shape in df['shape_v4'].unique() if len(shape) >= 3 and shape[2] == char]
        ...
        # 保存物理分类分析图
        filepath = os.path.join(self.output_dir, 'v4_physical_category_analysis.png')
        plt.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_shape_statistics(self, df):
        """创建shape统计图"""
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('V4 Shape分类统计分析', fontsize=16, fontweight='bold')
        ...
        top_shapes = df['shape_v4'].value_counts().head(10)
        ...
        # 保存统计图
        filepath = os.path.join(self.output_dir, 'v4_shape_statistics.png')
        plt.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def generate_all_shape_visualizations(self):
        ...
        # 生成shape分布统计
        shape_counts = df['shape_v4'].value_counts()
        print(f"发现 {len(shape_counts)} 种不同的V4 Shape类型")
        ...
        for shape in df['shape_v4']:
            if len(shape) >= 4:
                for i, char in enumerate(shape):
                    seg_name = f'段{i+1}'
                    segment_stats[seg_name][char] = segment_stats[seg_name].get(char, 0) + 1
