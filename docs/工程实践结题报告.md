...
4 张核心表（SQLAlchemy ORM，SQLite），详细字段见 4.6 节。
...
案例匹配策略（k=2，最多整形 2 次），严格训练/测试分割评估（案例库=训练集 2181，测试集 386 完全陌生，`scripts/eval_yield.py` 可复现）：

> 表 6-2　案例匹配良率评估（`scripts/eval_yield.py`，random_state=42）
