...
数据库 ER 图见图 4-4，4 张表及主要外键关系（`prediction_logs.feedback_id` ↔ `feedbacks.id`，`feedbacks.prediction_id` ↔ `prediction_logs.id`）。

```plantuml
@startuml 图4-4 数据库ER图
title 图 4-4　数据库 ER 图（4 张核心表）
hide circle
...
1. **方案优化**：当前案例匹配复用历史方案，未来可用模型反推最优压头参数（给定 Pre，搜索使 Post 合格的 rs_params）；
